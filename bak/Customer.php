...
class Customer extends MY_Controller {
    ...
    public function history(){
        ...
        $clear_type = $this->input->get('type');
        $clearday = $this->input->get('clearday');

        $this->db = $this->load->database('cisco',true);
        $sql = "select 
                sum(a.PayAmount) as TotalAmount, 													#总清算发票金额
                count(a.InvoiceId) as InvoiceCount, 												#发票数量
                count(distinct p.Vendorcode) as VendorCount,										#涉及供应商数量
                
                sum(CASE WHEN a.IsManual = 1 THEN 0 ELSE a.PayAmount END) as TotalAmount_A,			#平台清算金额
                sum(CASE WHEN a.IsManual = 1 THEN 0 ELSE a.PayDiscount END) as TotalDiscount_A,		#清算获得折扣金额
                sum(CASE WHEN a.IsManual = 0 THEN 0 ELSE a.PayAmount END) as TotalAmount_M,			#手工清算金额
                sum(CASE WHEN a.IsManual = 0 THEN 0 ELSE a.PayDiscount END) as TotalDiscount_M,		#清算获得折扣金额
				
                MIN(a.AwardDate) as FirstAwardDate,													#开始日期
                MAX(a.AwardDate) as LastAwardDate,													#结束日期
                MIN(a.PayDpe) as MinPayDPE,															#最小早付日期
                MAX(a.PayDpe) as MaxPayDPE,															#最大早付日期
                ROUND(AVG(a.PayDpe),1) as AvgPayDPE													#平均早付日期
				...
				where a.CashPoolId = {$marketid} and a.AwardStatus = 1";
        $query = $this->db->query($sql);
        $rs = $query->first_row('array');

        //折扣率、收益率
        $l_sql = "select a.PayAmount, a.PayDiscount , a.PayDpe , a.IsManual 
				FROM `Customer_PayAwards` a
				INNER JOIN `Customer_Payments` p ON p.Id = a.InvoiceId and p.InvoiceStatus = 2					# 发票只有 InvoiceStatus = 2 才为真正的成交结果				
				where a.CashPoolId = {$marketid} and a.AwardStatus = 1";
        $l_query = $this->db->query($l_sql);
        $l_rs = $l_query->result_array();
        $avg_apr = 0 ;			//平均年化收益率%
        ...
        foreach($l_rs as $row){
            $avg_apr += ROUND($row['PayDiscount'] / $row['PayDpe'] * 365 / $rs['TotalAmount'] * 100 , 2) ;
					if ($row['IsManual'] == 0) {
                        $avg_apr_a += ROUND($row['PayDiscount'] / $row['PayDpe'] * 365 / $rs['TotalAmount_A'] * 100 , 2) ;
						$avg_discount_a += ROUND($row['PayDiscount'] /$rs['TotalAmount_A'] * 100 , 2) ;
					} else {
                        $avg_apr_m += ROUND($row['PayDiscount'] / $row['PayDpe'] * 365 / $rs['TotalAmount_M'] * 100 , 2) ;
						$avg_discount_m += ROUND($row['PayDiscount'] /$rs['TotalAmount_M'] * 100 , 2) ;
					}
        }
        $rs['AvgApr'] = $avg_apr;
        $rs['AvgApr_A'] = $avg_apr_a;
        $rs['AvgDiscount_A'] = $avg_discount_a;
        $rs['AvgApr_M'] = $avg_apr_m;
        $rs['AvgDiscount_M'] = $avg_discount_m;
        $this->data['panel'] = $rs;

        //历史记录
        $clear_type = (!isset($clear_type)||$clear_type==-1)?"":"and a.IsManual = ".$clear_type;
        $award_date = (isset($start)&&$start!=""&&isset($end)&&$end!="")
            ?" and a.AwardDate between '".$start."' and '".$end."'"
            :" and a.AwardDate between '". date('Y-m-d',strtotime('-7 days'))."' and '".date('Y-m-d',time())."'";
        ...
        $h_sql = "select 
					...
					count(distinct p.Vendorcode) as VendorCount,#涉及供应商数量
					ROUND(AVG(a.PayDpe),1) as AvgPayDPE,		#平均早付日期
					sum(a.PayDiscount ) as TotalDiscount		#获得折扣金额
					FROM `Customer_PayAwards` a
					INNER JOIN `Customer_Payments` p ON p.Id = a.InvoiceId and p.InvoiceStatus = 2
					where a.CashPoolId = {$marketid} and a.AwardStatus = 1
					{$clear_type}{$award_date}
					group by a.AwardDate ORDER BY a.AwardDate DESC";
        $h_query = $this->db->query($h_sql);
        $h_rs = $h_query->result_array();
        $h_rs2 = array();
        foreach($h_rs as $item){
            $tmp_sql = "select a.PayAmount, a.PayDiscount , a.PayDpe , a.IsManual 
						FROM `Customer_PayAwards` a
						INNER JOIN `Customer_Payments` p ON p.Id = a.InvoiceId and p.InvoiceStatus = 2
						where a.CashPoolId = {$marketid} and a.AwardStatus = 1 AND a.AwardDate = '{$item["AwardDate"]}'".$clear_type;
            $tmp_query = $this->db->query($tmp_sql);
            $tmp_rs =$tmp_query->result_array();
            $avg_apr = 0 ;
            $avg_discount = 0 ;

            foreach( $tmp_rs as $row){
                $avg_apr +=ROUND($row['PayDiscount']/$row['PayDpe']*365/$item["TotalAmount"] * 100 , 2) ;
                $avg_discount += ROUND($row['PayDiscount']/$item["TotalAmount"]*100 , 2) ;
                }

            $item ["avg_apr"] = $avg_apr;
            $item ["avg_discount"] = $avg_discount;

            array_push($h_rs2,$item);
        }

        $this->data['history'] = $h_rs2;
