...
class UserlistModel extends CI_Model {
	...
	public function getInquiryUsersList($search_data = null,$num="", $offset=""){
		
		$this->db->select('*');
		$this->db->from('users_enquiry');
	...
	public function getInquiryUsersListCount($search_data = null,$num="", $offset=""){
	
		$this->db->select('*');
	    $this->db->from('users_enquiry as o');
		if($search_data != null && $search_data != ""){
		  $this->db->or_like($search_data);
		}
		$this->db->order_by('EnquiryId', 'ASC');
